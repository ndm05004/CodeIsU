...
// 업데이트 모달
function updateTask(taskNo, projBgngDt, projDdlnDt) {
  $.ajax({
    url: "/pms/project/taskDetail",
    data: {
      taskNo: taskNo,
    ...
    success: function (res) {
      ...
      document.querySelector("#progressPer").innerHTML = res.taskSttr + "%";
...
// 일감 생성 모달
function insertTask(projNo, projBgngDt, projDdlnDt) {
  if (projBgngDt == null && projDdlnDt == null) {
    projBgngDt = convertDate(projBgngDt);
...
function updateTaskSave(taskNo, header, token) {
  ...
  let taskUpdateSttr = $("#progress").val();
...
function insertTaskSave(projNo, header, token) {
  ...
  const taskinsertSttr = $("#progress").val();
  ...
  const HourMinute = currentHour + ":" + formattedMinute;
  ...
  const data = {
    ...
    taskCn: taskCn,
    upperTask: upperTask,
    taskSttr: taskinsertSttr,
    taskRegDt: HourMinute,
  };
...
document.addEventListener("click", function (event) {
  if (event.target.id === "insertTaskBtn") {
    insertTask(projNo, projBgngDt, projDdlnDt);
  }
});
...
document.addEventListener("click", function (event) {
  if (event.target.id === "updateTaskBtn") {
    ...
    console.log("updateTask : " + taskNo);
    updateTask(taskNo, projBgngDt, projDdlnDt);
  }
});
...
const taskAutoComplete = () => {
  document.getElementById("taskinsertTitle").value = "직원 인사 관리"; // 나중에 고쳐야함
  document.getElementById("taskinsertStts").value = "P003-1";
  $("#taskInsertPic").val("S2301201");
  document.getElementById("taskinsertBengDt").value = "2023-08-18"; // 나중에 고쳐야함
  document.getElementById("taskinsertDdlnDt").value = "2023-08-21"; // 나중에 고쳐야함
  document.getElementById("progress").value = 0;
  document.getElementById("progressPer").innerHTML = "0%";
  document.getElementById("taskinsertPriority").value = "P005-3";
  document.getElementById("taskinsertCn").innerText = "직원 인사이동 기능 일감"; // 나중에 고쳐야함
  document.getElementById("taskInsertTaskNo").value = "T777"; // 나중에 고쳐야함
};
